...
4. Un démarrage contrôlé sur la nouvelle image interroge le portefeuille DEMO et les ordres d'**ouverture** en attente. Si le broker expose encore des positions non connues du SQLite neuf, ou des ouvertures en attente, Goblin démarre **en observation uniquement** : bougies, logs, métriques et lecture d'equity restent possibles ; `new_risk_allowed=False` ; aucun BUY ; aucune clôture émise pour ces positions externes. Un marqueur durable `data/v3_external_broker_activity.json`, distinct du SQLite, est créé. **Ne pas l'effacer lors d'une remise à zéro SQLite.**
5. **Pas de reprise automatique des BUY**, même après disparition des positions du portefeuille. Le marqueur durable reste bloquant après un redémarrage. Quand l'opérateur a personnellement vérifié sur eToro DEMO **zéro position, zéro ouverture en attente ET clôtures manuelles exécutées ou annulées**, arrêter Goblin, sauvegarder le marqueur avec l'archive de l'incident, puis retirer **uniquement** `data/v3_external_broker_activity.json` par une opération explicite. Un nouveau démarrage effectue à nouveau les contrôles de portefeuille et d'ordres d'ouverture. Si le compte n'est pas plat, il récrée le marqueur et reste en observation.
