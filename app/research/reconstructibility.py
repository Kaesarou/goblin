...
from app.research.research_state import RESEARCH_BASE_FEATURE_NAMES
...
def feature_reconstructibility() -> dict[str, str]:
    mapping = {
        ...
        for name in RESEARCH_BASE_FEATURE_NAMES
    }
    mapping.update(
        {
            name: (
    ...
    return {
        ...
        'classes': {
            ...
            ResearchReconstructibility.HISTORICAL_PRICE_CHANGE_PROXY.value: (
                'May be reconstructed only from accepted price-changing '
                'events and must be named/labelled as a proxy.'
            ),
            ResearchReconstructibility.PROSPECTIVE_ONLY.value: (
        ...
        'family_classification': {
            'm1_candle_and_side_neutral_context': (
                ResearchReconstructibility.EXACT_HISTORICAL.value
            ),
        ...
        'historical_market_stream_scope': (
            'accepted price-changing snapshots only; unchanged WebSocket '
            'patch activity and patch field presence are not retained'
        ),
    }
