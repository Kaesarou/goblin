...
class V3RuntimeStateStore:
    ...
    def save_feature_engine(self, engine: OnlineFeatureEngine) -> None:
    ...
    def restore_feature_engine(self, engine: OnlineFeatureEngine) -> tuple[str, ...]:
        with self._connect() as connection:
    ...
    def restore_inventory_book(self, book: InventoryBook) -> tuple[str, ...]:
        ...
        for inventory_id, version, asof_text, payload_json in rows:
            ...
            if inventory is None or inventory.total_units <= 0:
                continue
            if str(version) != V3_RUNTIME_STATE_VERSION:
                raise RuntimeError(f"Unsupported V3 inventory runtime state: {version}")
            snapshot_asof = datetime.fromisoformat(str(asof_text))
            last_fill = inventory.last_fill_at or inventory.last_entry_at
            # A fill after the snapshot resets trailing state, so an older snapshot
            # must never overwrite the event-derived reset.
            if snapshot_asof < last_fill:
                continue
            payload = json.loads(str(payload_json))
            book._inventories[iid] = replace(  # projection-owned state
                inventory,
                ...
                min_price_since_open=_opt_float(payload.get("min_price_since_open")),
                max_price_since_open=_opt_float(payload.get("max_price_since_open")),
...
def legacy_v1_state_counts(path: str | Path) -> dict[str, int]:
    ...
    V3 deliberately performs no automatic migration from the old single-trade
    lifecycle. Existing rows require an explicit operator decision before V3 may
    start, otherwise the new runtime could ignore broker exposure it does not own.
    """
